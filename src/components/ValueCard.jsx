export default function ValueCard({ icon, title, desc, highlight }) {
  return (
    <div className={`p-5 lg:p-8 border ${highlight ? 'border-orange-600 bg-orange-600/5' : 'border-zinc-800 bg-zinc-900/50'} flex flex-col justify-center hover:bg-zinc-800 transition-colors group relative overflow-hidden`}>
      <div className={`mb-6 ${highlight ? 'text-orange-500' : 'text-zinc-500'} group-hover:text-orange-500 transition-colors`}>
        {icon}
      </div>
      <h3 className="font-bebas text-2xl lg:text-4xl tracking-wider text-white mb-3 lg:mb-4">{title}</h3>
